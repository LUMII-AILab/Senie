package LvSenie::Unicode::Win1257ToUnicodeTables;
use strict;
use utf8;
use warnings;

use Exporter();
use parent qw(Exporter);
our @EXPORT_OK = qw(substTable hasTable);

################################################################################
# Unikodifikācijas tabulas
# ------------------------
#
# Katra rinda atbilst vienam pārveides likumam: ko pāveidot (teksts), par ko
# pārveidot (teksts). Unikoda kodu norādīšana formā `x{kods}.

################################################################################

our $ST_SINGLES = {
	'AP1622'               => {
		'a´'  => "\x{00E1}",
		'a~'  => "\x{00E3}",
		'a^'  => "\x{00E2}",
		'a&'  => "\x{0227}",
		'ä'   => "\x{00E4}",
		'c´'  => "\x{0107}",
		'c&'  => "\x{010B}",
		'e´'  => "\x{00E8}",
		'e~'  => "\x{1EBD}",
		'e^'  => "\x{00EA}",
		'e#'  => "\x{0119}",
		'J#'  => "\x{0248}",
		'l#'  => "\x{0142}",
		'n´'  => "\x{0144}",
		'n~'  => "\x{00F1}",
		'o`'  => "\x{00F2}",
		'o´'  => "\x{00F3}",
		'o^'  => "\x{00F4}",
		'o~'  => "\x{00F5}",
		'o[]' => "\x{A76F}",
		'ö'   => "\x{00F6}",
		'§'   => "\x{017F}",
		's´'  => "\x{015B}",
		'u`'  => "\x{00F9}",
		'u^'  => "\x{00FB}",
		'u~'  => "\x{0169}",
		'ü'   => "\x{00FC}",
		'z´'  => "\x{017A}",
		'z&'  => "\x{017C}",
	},
	'Baum1699_LVV'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
	},
	'Baumb1795_WLWJD'      => {
		'ś'  => "\x{1E9C}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'i^' => "\x{00EE}",
		'a^' => "\x{00E2}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
	},
	'Bluhm1791_MWU'        => {
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'§'  => "\x{017F}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'e^' => "\x{00EA}",
	},
	'Br1520_PN'            => {
		'§'  => "\x{017F}",
		'a~' => "\x{00E3}",
		'e~' => "\x{1EBD}",
	},
	'Bruehn1756_DLWS'      => {
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
		'§'  => "\x{017F}",
		'Ś'  => "\x{A7A8}",
		'o^' => "\x{00F4}",
	},
	'CC1585'               => {
		'§'  => "\x{017F}",
		'u~' => "\x{0169}",
		'e~' => "\x{1EBD}",
	},
	'CekFJ1790_KD'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'e`' => "\x{00E8}",
		'Ś'  => "\x{A7A8}",
	},
	'CekFV1796_NL'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
	},
	'Depk1681_90_ms'       => {
		'ä'  => "\x{00E4}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'ö'  => "\x{00F6}",
		'§'  => "\x{017F}",
	},
	'Depk1703_TSD'         => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a^' => "\x{00E2}",
		'a`' => "\x{00E0}",
		'n~' => "\x{00F1}",
		'e^' => "\x{00EA}",
		'Ś'  => "\x{A7A8}",
		'i^' => "\x{00EE}",
	},
	'Depk1704_Vortr'       => {
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'a`' => "\x{00E0}",
		'e`' => "\x{00E8}",
		'o^' => "\x{00F4}",
		'Ś'  => "\x{A7A8}",
		'ņ~' => "ņ\x{0303}",
		'n~' => "\x{00F1}",
		'm~' => "m\x{0303}",
	},
	'Dres1682_SBM'         => {
		'§'   => "\x{017F}",
		'e~'  => "\x{1EBD}",
		'u~'  => "\x{0169}",
		'ś'   => "\x{1E9C}",
		'a\'' => "\x{00E1}",
		'a^'  => "\x{00E2}",
		'a`'  => "\x{00E0}",
		'm~'  => "m\x{0303}",
		'n~'  => "\x{00F1}",
		'a~'  => "\x{00E3}",
		'Ś'   => "\x{A7A8}",
	},
	'Eid1701_KB'           => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'a~' => "\x{00E3}",
		'm~' => "m\x{0303}",
	},
	'Eid1701_RA'           => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'o^' => "\x{00F4}",
	},
	'Elg1621_GCG'          => {
		'§'   => "\x{017F}",
		'o^'  => "\x{00F4}",
		'o`'  => "\x{00F2}",
		'o~'  => "\x{00F5}",
		'o´'  => "\x{00F3}",
		'a´'  => "\x{00E1}",
		'a^'  => "\x{00E2}",
		'a~'  => "\x{00E3}",
		'a`'  => "\x{00E0}",
		'u`'  => "\x{00F9}",
		'u^'  => "\x{00FB}",
		'u´'  => "\x{00FA}",
		'u~'  => "\x{0169}",
		'e^'  => "\x{00EA}",
		'e´'  => "\x{00E9}",
		'e~'  => "\x{1EBD}",
		'e`'  => "\x{00E8}",
		'n~'  => "\x{0144}",
		'm~'  => "m\x{0303}",
		'e\'' => "e",
	},
	'Elg1672_EvTA'         => {
		'a`'  => "\x{00E0}",
		'a´'  => "\x{00E1}",
		'a~'  => "\x{00E3}",
		'a^'  => "\x{00E2}",
		'ae'  => "\x{00E6}",
		'c\'' => "\x{0107}",
		'e`'  => "\x{00E8}",
		'e´'  => "\x{00E9}",
		'e^'  => "\x{00EA}",
		'i^'  => "\x{00EE}",
		'l#'  => "\x{0142}",
		'n~'  => "\x{00F1}",
		'o`'  => "\x{00F2}",
		'o\'' => "\x{00F3}",
		'o^'  => "\x{00F4}",
		'§'   => "\x{017F}",
		's#'  => "\x{015B}",
		'u^'  => "\x{00FB}",
		'z\'' => "\x{017A}",
	},
	'Elg1672_Cat'          => {
		'a`' => "\x{00E0}",
		'a´' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'e`' => "\x{00E8}",
		'e´' => "\x{00E9}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'ł'  => "\x{0142}",
		'o´' => "\x{00F3}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'u^' => "\x{00FB}",
		'z´' => "\x{017A}",
		'z&' => "\x{017C}",

	},
	'Elg1673_CS'           => {
		'a`' => "\x{00E0}",
		'a´' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'c´' => "\x{0107}",
		'e`' => "\x{00E8}",
		'e´' => "\x{00E9}",
		'e~' => "\x{1EBD}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0207}",
		'i&' => "\x{020B}",
		'l#' => "\x{0142}",
		'n´' => "\x{0144}",
		'n~' => "\x{00F1}",
		'o´' => "\x{00F3}",
		'o^' => "\x{00F4}",
		'ö'  => "\x{00F6}",
		's´' => "\x{015B}",
		'§'  => "\x{017F}",
		'ß'  => "\x{00DF}",
		'u^' => "\x{00FB}",
		'ü'  => "\x{00FC}",
		'z´' => "\x{017A}",
		'z&' => "\x{017C}",
	},
	'Elg1683_DictPLL'      => {
		'a`'  => "\x{00E0}",
		'a´'  => "\x{00E1}",
		'a~'  => "\x{00E3}",
		'a^'  => "\x{00E2}",
		'a&'  => "\x{0227}",
		'ae'  => "\x{00E6}",
		'e`'  => "\x{00E8}",
		'e´'  => "\x{00E9}",
		'e~'  => "\x{1EBD}",
		'e^'  => "\x{00EA}",
		'i`'  => "\x{00EC}",
		'i~'  => "\x{0129}",
		'i^'  => "\x{00EE}",
		'l/'  => "\x{0142}",
		'L/'  => "\x{0141}",
		'n~'  => "\x{00F1}",
		'o`'  => "\x{00F2}",
		'o^'  => "\x{00F4}",
		'o~'  => "\x{00F5}",
		'§'   => "\x{017F}",
		'Ś'   => "\x{A7A8}",
		'u`'  => "\x{00F9}",
		'u^'  => "\x{00FB}",
		'u~'  => "\x{0169}",
		'uo'  => "\x{016F}",
		'u<'  => "\x{016D}",
		'z&'  => "\x{017C}",
		'c´'  => "\x{0107}",
		's´'  => "\x{015B}",
		'y~'  => "\x{1EF9}",
		'z´'  => "\x{017A}",
		'n´'  => "\x{0144}",
		'e<'  => "\x{0115}",
		'c&'  => "\x{010B}",
		'u<'  => "\x{016D}",
		'o#'  => "\x{A76F}",
		'e#'  => "\x{0119}",
		'a#'  => "\x{0105}",
		'ae^' => "æ\x{0302}",
		'Ae^' => "Æ\x{0302}",
		'q´'  => "q\x{0301}",
		'c`'  => "c\x{0300}",
		'X#'  => "\x{04FE}",
	},
	'Ench1586'             => {
		'§'  => "\x{017F}",
		'e~' => "\x{1EBD}",
		'n~' => "\x{00F1}",
		'm~' => "m\x{0303}",
		'a~' => "\x{00E3}",
	},
	'Ench1615'             => {
		'§'   => "\x{017F}",
		'e~'  => "\x{1EBD}",
		'n~'  => "\x{00F1}",
		'm~'  => "m\x{0303}",
		'a~'  => "\x{00E3}",
		'o\'' => "\x{00F6}",
	},
	'EvEp1587'             => {
		'§'  => "\x{017F}",
		'e~' => "\x{1EBD}",
		'n~' => "\x{00F1}",
		'a~' => "\x{00E3}",
		'u~' => "\x{0169}",
		'm~' => "m\x{0303}",
	},
	'EvEp1615'             => {
		'§'  => "\x{017F}",
		'e~' => "\x{1EBD}",
		'n~' => "\x{00F1}",
		'a~' => "\x{00E3}",
		'u~' => "\x{0169}",
		'm~' => "m\x{0303}",
	},
	'EvTA1753'             => {
		'§'         => "\x{017F}",
		'a^'        => "\x{00E2}", # NB! Prefix of other rule.
		'i^'        => "\x{00EE}",
		'o^'        => "\x{00F4}",
		'a`'        => "\x{00E0}",
		'l`'        => "\x{0142}",
		'e^'        => "\x{00EA}",
		'u`'        => "\x{00F9}",
		'i`'        => "\x{00EC}",
		'ž'         => "\x{017C}",
		'o`'        => "\x{00F2}",
		'i\''       => "\x{00ED}",
		'e`'        => "\x{00E8}",
		'o~'        => "\x{00F5}",
		'u^'        => "\x{00FB}",
		'ņ'         => "\x{0144}",
		'e\''       => "\x{00E9}",
		'o\''       => "\x{00F3}",
		'u\''       => "\x{00FA}",
		'a~'        => "\x{00E3}",
		'u~'        => "\x{0169}",
		'L`'        => "\x{0141}",
		'a\''       => "\x{00E1}",
		'i~'        => "\x{0129}",
		'a^`'       => "\x{00E2}", #NB! Has prefix that is other rule.
		'\x{00E2}`' => "\x{00E2}", #NB! To mitigate prefixing problems.
	},
	'Fuer1650_70_1ms'      => {
		'n~'  => "\x{00F1}",
		'm~'  => "m\x{0303}",
		'a~'  => "\x{00E3}",
		'a^'  => "\x{00E2}",
		'ś'   => "\x{1E9C}",
		'o^'  => "\x{00F4}",
		'a`'  => "\x{00E0}",
		'i^'  => "\x{00EE}",
		'e^'  => "\x{00EA}",
		'o`'  => "\x{00F2}",
		't^'  => "\x{00EA}", # Source error
		'ņ~'  => "ņ\x{0303}",
		'e*'  => "\x{011B}",
		'Ś'   => "\x{A7A8}",
		'I^'  => "\x{00CE}",
		'ß/'  => "ß\x{0338}",
		'e"'  => "\x{00EB}",
		'y"'  => "\x{00FF}",
		'i"'  => "\x{00EF}",
		'o~'  => "\x{00F5}",
		'e~'  => "\x{1EBD}",
		'e`'  => "\x{00E8}",
		'u~'  => "\x{0169}",
		'u^'  => "\x{00FB}",
		'e\'' => "\x{00E9}",
		'y\'' => "\x{00FF}",
		'o\'' => "\x{00F6}",
		'b/'  => "\x{0180}",
		'f/'  => "\x{A799}",
		'd/'  => "\x{0111}",
		'l/'  => "\x{0142}",
		'R/'  => "\x{024C}",
		'q/'  => "\x{A757}",
		'p/'  => "\x{A751}",
		'p~'  => "p\x{0303}",
		'a/'  => "a\x{0338}",
		'v/'  => "v\x{0338}",
		'q~'  => "q\x{0303}",
	},
	'Fuer1650_70_2ms'      => {
		'm~'         => "m\x{0303}",
		'a^'         => "\x{00E2}",
		'ś'          => "\x{1E9C}", # NB! Overlapping rule
		'o^'         => "\x{00F4}",
		'a`'         => "\x{00E0}",
		'i^'         => "\x{00EE}",
		'e^'         => "\x{00EA}",
		'Ś'          => "\x{A7A8}",
		'i`'         => "\x{00EC}",
		'e`'         => "\x{00E8}",
		'I^'         => "\x{00CE}",
		'e#'         => "\x{011B}",
		'Z/'         => "\x{01B5}",
		'z/'         => "\x{01B6}",
		'a\''        => "\x{0227}",
		'u\''        => "u\x{0307}",
		'e\"'        => "u\x{00EB}",
		'a,'         => "a\x{0317}",
		'ą'          => "a\x{0317}",
		'y"'         => "\x{00FF}",
		'i"'         => "\x{00EF}",
		'ß/'         => "ß\x{0338}",
		'a~'         => "\x{00E3}",
		'o~'         => "\x{00F5}",
		'e~'         => "\x{1EBD}",
		'u~'         => "\x{0169}",
		'u^'         => "\x{00FB}",
		'o#'         => "\x{00F6}",
		'e\''        => "\x{00E9}",
		'o\''        => "o",
		'w/'         => "w",
		'q/'         => "\x{A757}",
		'p/'         => "\x{A751}",
		'R/'         => "\x{024C}",
		'd/'         => "\x{0111}",
		'I/'         => "\x{0197}",
		'l/'         => "\x{0142}",
		'b/'         => "\x{0180}",
		'f/'         => "\x{A799}",
		'h/'         => "\x{0127}",
		'oi\''       => "i\x{022F}",        # Error correction
		'n\"'        => "\x{00FC}",         # Error correction
		'o\''        => "o",                # Error correction
		'ś\"'        => "\x{1E9C}\x{00EE}", # Error correction, overlapping rule
		'\x{1E9C}\"' => "\x{1E9C}\x{00EE}", # Error correction, overlapping rule
		'o`'         => "\x{00F2}",
		'm`'         => "m\x{0340}",
		'n~'         => "\x{00F1}",
		'q#'         => "q\x{0303}",
		'q^'         => "q\x{0302}",

	},
	'Fuhr1690_LL'          => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'i#' => "",
	},
	'Gis1507_PN'           => {
		'§'  => "\x{017F}",
		'z'  => "\x{0292}",
		'u~' => "\x{0169}",
	},
	'GD_1698'              => {
		'§'  => "\x{017F}",
		'o^' => "\x{00F4}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'i^' => "\x{00EE}",
		'a`' => "\x{00E0}",
		'Ś'  => "\x{A7A8}",
		'e^' => "\x{00EA}",
		'i`' => "\x{00EC}",
	},
	'Glueck1699_SBM'       => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'a`' => "\x{00E0}",
		'i^' => "\x{00EE}",
		'e`' => "\x{00E8}",
		'i`' => "\x{00EC}",
		'o^' => "\x{00F4}",
		'e^' => "\x{00EA}",
		't^' => "t\x{00E2}", # error correction t^ -> ta^
		'a#' => "\x{00E4}",
		'e#' => "\x{00EB}",
		'a´' => "\x{00E1}",
		'u^' => "\x{00FB}",
		'Ś'  => "\x{A7A8}",
	},
	'Gr1520_PN'            => {
		'§'  => "\x{017F}",
		'y#' => "\x{00FF}",
		's~' => "m\x{0303}"
	},
	'Hag1790_IM'           => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'a`' => "\x{00E0}",
		'e`' => "\x{00E8}",
	},
	'Has1550_PN'           => {
		'§'  => "\x{017F}",
		'uo' => "\x{016F}",
	},
	'Klein1672_LPG'        => {
		'a`' => "\x{00E0}",
		'a~' => "\x{00E3}",
		'a^' => "\x{00E2}",
		'ä'  => "\x{00E4}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'o^' => "\x{00F4}",
		'ö'  => "\x{00F6}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u~' => "\x{0169}",
		'ü'  => "\x{00FC}",
	},
	'Lang1685_LDL_ms'      => {
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'ā'  => "\x{0101}",
		'ä'  => "\x{00E4}",
		'Ä'  => "\x{00C4}",
		'e`' => "\x{00E8}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'o`' => "\x{00F2}",
		'O`' => "\x{00D2}",
		'ö'  => "\x{00F6}",
		'Ö'  => "\x{00D6}",
		'§'  => "\x{017F}",
		'ś'  => "\x{015B}",
		'Ś'  => "\x{A7A8}",
		'u~' => "\x{0169}",
		'ü'  => "\x{00FC}",
		'y#' => "\x{00FF}",
		'R#' => "\x{211E}",
		'x2' => "\x{00D7}",
	},
	'Laz1557_PN'           => {}, # No special symbols in this file.
	'LGL1685_K1'           => {
		'§'   => "\x{017F}",
		'ś'   => "\x{1E9C}",
		'a^'  => "\x{00E2}",
		'i^'  => "\x{00EE}",
		'a`'  => "\x{00E0}",
		'o^'  => "\x{00F4}",
		'e^'  => "\x{00EA}",
		'Ś'   => "\x{A7A8}",
		'u\'' => "u\x{0307}",
		'r\'' => "\x{0156}",
		'u^'  => "\x{00FB}",
		'e`'  => "\x{0117}",
		'u`'  => "u\x{0307}",
		'a\'' => "\x{00E0}",
		'e~'  => "\x{00EB}",
		'o`'  => "\x{00F2}",
		'm~'  => "m\x{0303}",
	},
	'LGL1685_V5'           => {
		'§'   => "\x{017F}",
		'ś'   => "\x{1E9C}",
		'a^'  => "\x{00E2}",
		'i^'  => "\x{00EE}",
		'a`'  => "\x{00E0}",
		'o^'  => "\x{00F4}",
		'a&'  => "\x{0227}",
		'e^'  => "\x{00EA}",
		'n~'  => "\x{00F1}",
		'Ś'   => "\x{A7A8}",
		'u^'  => "\x{00FB}",
		'e~'  => "\x{1EBD}",
		'a\'' => "\x{00E1}",
		'a~'  => "\x{00E3}",
		'i~'  => "\x{0129}",
		'g\'' => "\x{0123}",
		'm~'  => "m\x{0303}",
		'u~'  => "\x{0169}",
		'e`'  => "\x{00E8}",
	},
	'Lod1778_WTMD'         => {
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'Ś'  => "\x{A7A8}",
		'§'  => "\x{017F}",
		'i^' => "\x{00EE}",
		'e^' => "\x{00EA}",
		'e#' => "\x{00EB}",
		'a`' => "\x{00E0}",
		'e´' => "\x{00E9}",
		'u^' => "\x{00FB}",
	},
	'Lod1775_SEAPP'        => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'e´' => "\x{00E9}",
	},
	'Lop1800_SDLS'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'e`' => "\x{00E8}",
		'u^' => "\x{00FB}",
		'a`' => "\x{00E0}",
		'k^' => "k",
		'n^' => "n",
	},
	'LPDD1704'             => {
		'a`' => "\x{00E0}",
		'a´' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a~' => "\x{00E3}",
		'e^' => "\x{00EA}",
		'e~' => "\x{1EBD}",
		'i^' => "\x{00EE}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'o^' => "\x{00F4}",
		'ö'  => "\x{00F6}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'*'  => "\x{002A}",
		'†'  => "\x{2020}",
	},
	'LS1625'               => {
		'§' => "\x{017F}",
	},
	'Manc1631_Cat'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a`' => "\x{00E0}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'm~' => "m\x{0303}",
	},
	'Manc1631_LGL'         => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a`' => "\x{00E0}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'm~' => "m\x{0303}",
		'e#' => "\x{00EB}",
		#'t^' => "\x{?}",
	},
	'Manc1631_LVM'         => {
		'§'     => "\x{017F}",
		'a`'    => "\x{00E0}",
		'ś'     => "\x{1E9C}",
		'n~'    => "\x{00F1}",
		'Ś'     => "\x{A7A8}",
		'o^'    => "\x{00F4}",
		'a^'    => "\x{00E2}",
		'e`'    => "\x{00E8}",
		'o`'    => "\x{00F2}",
		'u`'    => "\x{00F9}",
		'm~'    => "m\x{0303}",
		'e#'    => "\x{00EB}",
		'e\"'   => "\x{00EB}",
		'e\'\'' => "\x{00EB}",
		'q\''   => "q\x{0300}",
	},
	'Manc1631_Syr'         => {
		'§'   => "\x{017F}",
		'ś'   => "\x{1E9C}",
		'a`'  => "\x{00E0}",
		'n~'  => "\x{00F1}",
		'Ś'   => "\x{A7A8}",
		'm~'  => "m\x{0303}",
		'e`'  => "\x{00E8}",
		'o\'' => "\x{00F3}",
		'o^'  => "\x{00F4}",
	},
	'Manc1637_Sal'         => {
		'ś'   => "\x{1E9C}",
		'§'   => "\x{017F}",
		'a`'  => "\x{00E0}",
		'n~'  => "\x{00F1}",
		'Ś'   => "\x{A7A8}",
		'm~'  => "m\x{0303}",
		'o`'  => "\x{00F2}",
		'o^'  => "\x{00F4}",
		'e\'' => "\x{00E8}",
	},
	'Manc1638_L'           => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a`' => "\x{00E0}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'm~' => "m\x{0303}",
		'e~' => "\x{1EBD}",
		'u~' => "\x{0169}",
		'a~' => "\x{00E3}",
		'o`' => "\x{00F2}",
		'e`' => "\x{00E8}",
		'o^' => "\x{00F4}",
		'a^' => "\x{00E2}",
	},
	'Manc1638_PhL'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a`' => "\x{00E0}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'e~' => "\x{1EBD}",
		'u~' => "\x{0169}",
		'm~' => "m\x{0303}",
		'o^' => "\x{00F4}",
		'a^' => "\x{00E2}",
		'e`' => "\x{00E8}",
	},
	'Manc1638_Run'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a`' => "\x{00E0}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'm~' => "m\x{0303}",
		'e~' => "\x{1EBD}",
		'u~' => "\x{0169}",
		'a~' => "\x{00E3}",
		'o~' => "\x{00F5}",
	},
	'Manc1643_44_Cat'      => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'e~' => "\x{1EBD}",
		'a`' => "\x{00E0}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'a&' => "\x{0227}",
		'a~' => "\x{00E3}",
		'Ś'  => "\x{A7A8}",
	},
	'Manc1643_44_LVM'      => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a´' => "\x{00E1}",
		'e~' => "\x{1EBD}",
		'm~' => "m\x{0303}",
		'a~' => "\x{00E3}",
		'n~' => "\x{00F1}",
		# 'e~' => "\x{1EBD}",
		'e#' => "\x{00EB}",
		'Ś'  => "\x{A7A8}",
		'e`' => "\x{00E8}",
		'a`' => "\x{00E0}",
		'Ö'  => "\x{00F6}",
	},
	'Manc1643_LGL'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a`' => "\x{00E0}",
		'e~' => "\x{1EBD}",
		'a~' => "\x{00E3}",
		'n~' => "\x{00F1}",
		'm~' => "m\x{0303}",
		'e#' => "\x{00EB}",
		'a´' => "\x{00E1}",
		'Ś'  => "\x{A7A8}",
	},
	'Manc1643_Syr'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a`' => "\x{00E0}",
		'a~' => "\x{00E3}",
		'a´' => "\x{00E1}",
		'n~' => "\x{00F1}",
		'm~' => "m\x{0303}",
		'Ś'  => "\x{A7A8}",
	},
	'Manc1654_LP1'         => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a`' => "\x{00E0}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'a~' => "\x{00E3}",
		'm~' => "m\x{0303}",
		'e~' => "\x{1EBD}",
		'e#' => "\x{00EB}",
		'o^' => "\x{00F4}",
		'n`' => "\x{00F1}",
		'o`' => "o",
	},
	'Manc1654_LP2'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a`' => "\x{00E0}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'a~' => "\x{00E3}",
		'm~' => "m\x{0303}",
		'a#' => "\x{00E4}",
		'u~' => "\x{0169}",
		'e#' => "\x{00EB}",
		'e`' => "\x{00E8}",
	},
	'Manc1654_LP3'         => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a`' => "\x{00E0}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'a~' => "\x{00E3}",
		'm~' => "m\x{0303}",
		'e~' => "\x{1EBD}",
	},
	'MD1788'               => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'e#' => "\x{00EB}",
		'i#' => "\x{00EF}",
		'o#' => "\x{00F6}",
		'Ś'  => "\x{A7A8}",
	},
	'Meg1593_PN'           => {
		'§'         => "\x{017F}", #NB: note the interfering rules
		'u~'        => "\x{0169}",
		'e~'        => "\x{1EBD}",
		'§t'        => "\x{FB05}", #NB: note the interfering rules
		"\x{017F}t" => "\x{FB05}", #NB: note the interfering rules
		'a~'        => "\x{00E3}",
	},
	'Ps1615'               => {
		'§'  => "\x{017F}",
		'e~' => "\x{1EBD}",
		'n~' => "\x{00F1}",
		'm~' => "m\x{0303}",
		'a~' => "\x{00E3}",
		'u~' => "\x{0169}",
	},
	'Rav1767_SD'           => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'e`' => "\x{00E8}",
	},
	'Reit1675_OD'          => {
		'§'  => "\x{017F}",
		'e`' => "\x{00E8}",
	},
	'Reit1675_UeP'         => {
		'§'  => "\x{017F}",
		#'ś'  => "\x{1E9C}",
		'ś'  => "\x{017F}",
		'a~' => "\x{00E3}",
		'n~' => "\x{00F1}",
		'm~' => "m\x{0303}",
		'e~' => "\x{1EBD}",
		'a`' => "\x{00E0}",
	},
	'SKL1696_KB'           => {
		'§'   => "\x{017F}",
		'ś'   => "\x{1E9C}",
		'a^'  => "\x{00E2}",
		'o^'  => "\x{00F4}",
		'e^'  => "\x{00EA}",
		'i^'  => "\x{00EE}",
		'a\'' => "\x{0227}",
		'Ś'   => "\x{A7A8}",
		'm~'  => "m\x{0303}",
	},
	'SKL1696_RA'           => {
		'§'   => "\x{017F}",
		'ś'   => "\x{1E9C}",
		'a^'  => "\x{00E2}",
		'o^'  => "\x{00F4}",
		'a`'  => "\x{00E0}",
		'e^'  => "\x{00EA}",
		'i^'  => "\x{00EE}",
		'a\'' => "\x{0227}",
		'Ś'   => "\x{A7A8}",
	},
	'SL1684'               => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'Ś'  => "\x{A7A8}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'i^' => "\x{00EE}",
	},
	'SL1789'               => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'o`' => "\x{00F2}",
		'e`' => "\x{00E8}",
		'u^' => "\x{00FB}",
	},
	'SLM1648'              => {
		'§'  => "\x{017F}",
		'a´' => "\x{00E1}",
		's´' => "\x{015B}",
	},
	'StendAJ1790_LSMP'     => {
		'§'     => "\x{017F}",
		'ś'     => "\x{1E9C}",
		'a^'    => "\x{00E2}",
		'o^'    => "\x{00F4}",
		'e^'    => "\x{00EA}",
		'i^'    => "\x{00EE}",
		'Ś'     => "\x{A7A8}",
		'a`'    => "\x{00E0}",
		'e`'    => "\x{00E8}",
		'i\'\'' => "\x{00EF}",
		'e\'\'' => "\x{00EB}",
		'u^'    => "\x{00FB}",
	},
	'StendAJ1793_JGW'      => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'u#' => "\x{00FC}",
		'Ś'  => "\x{A7A8}",
	},
	'StendGF1774_AGG'      => {
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'e`' => "\x{00E8}",
		'e^' => "\x{00EA}",
		'e#' => "\x{00EB}",
		'i^' => "\x{00EE}",
		'i#' => "\x{00EF}",
		'i"' => "\x{00EF}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
	},
	'StendGF1781_JGW'      => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'i^' => "\x{00EE}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'Ś'  => "\x{A7A8}",
		'e`' => "\x{00E8}",
	},
	'StendGF1789_SL'       => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'e`' => "\x{00E8}",
		'a´' => "\x{00E1}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
	},
	'Stobbe1796_PTK'       => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
	},
	'Sulc1764_ARMST'       => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
	},
	'Tetsch1784_DKTW'      => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'Ś'  => "\x{A7A8}",
	},
	'Thev1575_PN'          => {
		'§' => "\x{017F}",
		#'ß'  => "\x{00DF}",		#00FD->00FD
	},
	'TII1790'              => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
	},
	'UKG1693'              => {
		'a`' => "\x{00E0}",
		'ä'  => "\x{00E4}",
		'o^' => "\x{0044}",
		'ö'  => "\x{00F6}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'UKG1696'              => {
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'ö'  => "\x{00F6}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'UP1587'               => { #TODO
		'§'         => "\x{017F}",
		'e~'        => "\x{1EBD}",
		'n~'        => "\x{00F1}", #NB: note the interfering rules
		'en~'       => "\x{1EBD}", #NB: note the interfering rules
		"e\x{00F1}" => "\x{1EBD}", #NB: note the interfering rules
		'a~'        => "\x{00E3}", #NB: note the interfering rules
		'an~'       => "\x{00E3}", #NB: note the interfering rules
		"a\x{00F1}" => "\x{00E3}", #NB: note the interfering rules
	},
	'Urban1791_LKLDD'      => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'o^' => "\x{00F4}",
		'a^' => "\x{00E2}",
		'a`' => "\x{00E0}",
		'e`' => "\x{00E8}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
	},
	'V1771_SZA'            => {
		'a^' => "\x{00E2}",
		'Ś'  => "\x{A7A8}",
		'n~' => "\x{00F1}",
	},
	'VLH1685'              => {
		'§'   => "\x{017F}",
		'ś'   => "\x{1E9C}",
		'a^'  => "\x{00E2}",
		'u^'  => "\x{00FB}",
		'e#'  => "\x{00EB}",
		'a#'  => "\x{00E4}",
		'i^'  => "\x{00EE}",
		'a`'  => "\x{00E0}",
		'u&'  => "u\x{0307}",
		'e&'  => "\x{0117}",
		'o^'  => "\x{00F4}",
		'a&'  => "\x{0227}",
		'e^'  => "\x{00EA}",
		'e\'' => "\x{00E9}",
		'e`'  => "\x{00E8}",
		'a´'  => "\x{00E1}",
		'Ś'   => "\x{A7A8}",
		'o&'  => "\x{022F}",
		'n~'  => "\x{00F1}",
		'a~'  => "\x{00E3}",
		'g^'  => "g\x{00E2}",
		'm~'  => "m\x{0303}",
	},
	'VLH1685_Cat'          => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'i^' => "\x{00EE}",
		'a`' => "\x{00E0}",
		'u&' => "u\x{0307}",
		'e&' => "\x{0117}",
		'o&' => "\x{022F}",
		'o^' => "\x{00F4}",
		'a&' => "\x{0227}",
		#'ä' => "a\x{0364}",
		'e^' => "\x{00EA}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
	},
	'VLH1685_Sal'          => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'e#' => "\x{00EB}",
		'i^' => "\x{00EE}",
		'a`' => "\x{00E0}",
		'u&' => "u\x{0307}",
		'e&' => "\x{0117}",
		'o^' => "\x{00F4}",
		'a&' => "\x{0227}",
		'e^' => "\x{00EA}",
		'Ś'  => "\x{A7A8}",
	},
	'VLH1685_Syr'          => {
		'§'   => "\x{017F}",
		'ś'   => "\x{1E9C}",
		'a^'  => "\x{00E2}",
		'u^'  => "\x{00FB}",
		'e#'  => "\x{00EB}",
		'a#'  => "\x{00E4}",
		'i^'  => "\x{00EE}",
		'a`'  => "\x{00E0}",
		'u&'  => "u\x{0307}",
		'e&'  => "\x{0117}",
		'o^'  => "\x{00F4}",
		'a&'  => "\x{0227}",
		'e^'  => "\x{00EA}",
		'e`'  => "\x{00E8}",
		'a~'  => "\x{00E3}",
		'ar&' => "r\x{0227}",
		'n~'  => "\x{00F1}",
		'o&'  => "\x{022F}",
		'Ś'   => "\x{A7A8}",
	},
	'Witt1696_MMID'        => {
		'§'  => "\x{017F}",
		'a~' => "\x{00E3}",
	},
	'Witt1702_PAN'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'a`' => "\x{00E0}",
		'u^' => "\x{00FB}",
		'Ś'  => "\x{A7A8}",
	},
	'ZP1685'               => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'Ś'  => "\x{A7A8}",
	},
	'Zv1638_VAR'           => {
		'§' => "\x{017F}",
	},
	'Zv1681_Kok'           => {
		'§'  => "\x{017F}",
		'e`' => "\x{00E8}",
	},
	'Zv1681_Liec_1'        => {
		'§'  => "\x{017F}",
		'o~' => "\x{00F5}",
	},
	'Zv1681_Liec_2'        => {
		'§' => "\x{017F}",
	},
	'Zv1689_Kan'           => {
		'§'  => "\x{017F}",
		'e`' => "\x{00E8}",
	},
	'Zv1689_Salsnes'       => {
		'§'  => "\x{017F}",
		'm~' => "m\x{0303}",
	},
	'Zv1698_Lig'           => {
		'§'  => "\x{017F}",
		'n~' => "\x{00F1}",
		'm~' => "m\x{0303}",
		'e`' => "\x{00E8}",
	},
	'LVVV_ENTRIES_UNIFIED' => {
		'a`'  => "\x{00E0}",
		'a´'  => "\x{00E1}",
		'a\'' => "\x{00E1}",
		'a~'  => "\x{00E3}",
		'a^'  => "\x{00E2}",
		'a&'  => "\x{0227}",
		'A&'  => "\x{0226}",
		#'a#'  => "\x{00E4}",
		'ä'   => "\x{00E4}",
		'a,'  => "a\x{0371}",
		'a/'  => "a\x{0338}",
		#'a#'  => "\x{0105}",
		'æ'   => "\x{00E6}",
		'ae^' => "æ\x{0302}",
		'Ae^' => "æ\x{0302}",
		'ae'  => "\x{00E6}",
	# TODO: pabeigt, kad atsūtīs salaboto, viennozīmīgo tabulu.
	},
};

our $ST_Apokr1689 = {
	'1Mak'    => {
		'a`'  => "\x{00E0}",
		'a\'' => "\x{00E1}",
		'a^'  => "\x{00E2}",
		'a#'  => "\x{00E4}",
		'e^'  => "\x{00EA}",
		'e&'  => "\x{0117}",
		'e#'  => "\x{00EB}",
		'i`'  => "\x{00EC}",
		'i^'  => "\x{00EE}",
		'm~'  => "m\x{0303}",
		'o`'  => "\x{00F2}",
		'o^'  => "\x{00F4}",
		'o#'  => "\x{00F6}",
		'§'   => "\x{017F}",
		'ś'   => "\x{1E9C}",
		'Ś'   => "\x{A7A8}",
		'u&'  => "u\x{0307}",
		'u#'  => "\x{00FC}",
		'oh^' => "\x{00F4}h"	# Kļūdas labojums
	},
	'2Mak'    => {
		'a`' => "\x{00E0}",
		'a\'' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'a#' => "\x{00E4}",
		'e^' => "\x{00EA}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'o^' => "\x{00F4}",
		'o#' => "\x{00F6}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
	},
	'Asar'    => {
		'a`' => "\x{00E0}",
		'a\'' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'e^' => "\x{00EA}",
		'e#' => "\x{00EB}",
		'i^' => "\x{00EE}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
	},
	'Bar'     => {
		'a`' => "\x{00E0}",
		'a\'' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'a#' => "\x{00E4}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'Bel'     => {
		'a`' => "\x{00E0}",
		'a\'' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'GabpEst' => {
		'a`' => "\x{00E0}",
		'a\'' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i^' => "\x{00EE}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'Jd'      => {
		'a`' => "\x{00E0}",
		'a´' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
	},
	'Man'     => {
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'a#' => "\x{00E4}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'P_Sir'   => {
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'e#' => "\x{00EB}",
		'i^' => "\x{00EE}",
		'n~' => "\x{00F1}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'Prolog'  => {
		'e#' => "\x{00EB}",
		#'o#' => "\x{00F6}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
	},
	'Sal'     => {
		'a`' => "\x{00E0}",
		'a´' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
	},
	'Sir'     => {
		'a`' => "\x{00E0}",
		'a\'' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'a#' => "\x{00E4}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'm~' => "m\x{0303}",
		'ņ^' => "ņ\x{00E2}",	#datu kļūdas labojums
		'n~' => "\x{00F1}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
	},
	'Sus'     => {
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'e^' => "\x{00EA}",
		'e#' => "\x{00EB}",
		'i^' => "\x{00EE}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
	},
	'Tob'     => {
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'a#' => "\x{00E4}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
	},
};

our $ST_JT1685 = {
	'1J'         => {
		'a^' => "\x{00E2}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'i^' => "\x{00EE}",
		'a`' => "\x{00E0}",
		'a&' => "\x{0227}",
		'e^' => "\x{00EA}",
		'a~' => "\x{00E3}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
	},
	'1Kor'       => {
		'a^' => "\x{00E2}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a&' => "\x{0227}",
		'i^' => "\x{00EE}",
		#'ģ' => "ģ", ???
		'a`' => "\x{00E0}",
		'e&' => "\x{0117}",
		'u&' => "u\x{0307}",
		'n~' => "\x{00F1}",
		'e^' => "\x{00EA}",
		'Ś'  => "\x{A7A8}",
		'e~' => "\x{1EBD}",
		'u^' => "\x{00FB}",
	},
	'1P'         => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'i^' => "\x{00EE}",
		'a`' => "\x{00E0}",
		'i`' => "\x{00EC}",
		'o^' => "\x{00F4}",
		'n~' => "\x{00F1}",
		'e^' => "\x{00EA}",
		'Ś'  => "\x{A7A8}",
		'e~' => "\x{1EBD}",
		'm~' => "m\x{0303}",
		'a~' => "\x{00E3}",
	},
	'1Tes'       => {
		'a^' => "\x{00E2}",
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'e^' => "\x{00EA}",
		'a`' => "\x{00E0}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
		'o^' => "\x{00F4}",
	},
	'1Tim'       => {
		'a^' => "\x{00E2}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'i`' => "\x{00EC}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'u&' => "u\x{0307}",
		'a`' => "\x{00E0}",
		'o`' => "\x{00F2}",
		'o^' => "\x{00F4}",
		'a~' => "\x{00E3}",
		'i^' => "\x{00EE}",
		'Ś'  => "\x{A7A8}",
	},
	'2J'         => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
	},
	'2Kor'       => {
		'§'          => "\x{017F}",
		'ś'          => "\x{1E9C}", #NB: note the interfering rules
		'a&'         => "\x{0227}",
		'o^'         => "\x{00F4}",
		'a^'         => "\x{00E2}",
		'i^'         => "\x{00EE}",
		'e&'         => "\x{0117}",
		'e#'         => "\x{00EB}",
		'a`'         => "\x{00E0}",
		'i`'         => "\x{00EC}",
		'e^'         => "\x{00EA}",
		'n~'         => "\x{00F1}",
		'm~'         => "m\x{0303}",
		'a~'         => "\x{00E3}",
		'Ś'          => "\x{A7A8}",
		'e~'         => "\x{1EBD}",
		'ā'          => "a",         # error correction
		'ś\''        => "\x{1E9C}-", # error correction
		"\x{1E9C}\'" => "\x{1E9C}-", # error correction, overlapping rule
	},
	'2P'         => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a&' => "\x{0227}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'a~' => "\x{00E3}",
		'Ś'  => "\x{A7A8}",
	},
	'2Tes'       => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a^' => "\x{00E2}",
		'a`' => "\x{00E0}",
		'i^' => "\x{00EE}",
		'i`' => "\x{00EC}",
		'Ś'  => "\x{A7A8}",
	},
	'2Tim'       => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'i`' => "\x{00EC}",
		'u&' => "u\x{0307}",
		'a`' => "\x{00E0}",
		'o^' => "\x{00F4}",
		'o&' => "\x{022F}",
		'a~' => "\x{00E3}",
		'Ś'  => "\x{A7A8}",
		'i^' => "\x{00EE}",
	},
	'3J'         => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a~' => "\x{00E3}",
		'a`' => "\x{00E0}",
		'i^' => "\x{00EE}",
		'u~' => "\x{0169}",
		'a^' => "\x{00E2}",
		'Ś'  => "\x{A7A8}",
	},
	'Apd'        => {
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'§'  => "\x{017F}",
		'a~' => "\x{00E3}",
		'a&' => "\x{0227}",
		'e~' => "\x{1EBD}",
		'i^' => "\x{00EE}",
		'e#' => "\x{00EB}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'u^' => "\x{00FB}",
		'e`' => "\x{00E8}",
		'a#' => "\x{0227}", #Yes, the same as a&
		'm~' => "m\x{0303}",
		'u&' => "u\x{0307}",
		'o&' => "\x{022F}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'o`' => "\x{00F2}",
		'u~' => "\x{0169}",
		'u`' => "\x{00F9}",
	},
	'Atk'        => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'i^' => "\x{00EE}",
		'a&' => "\x{0227}",
		'e^' => "\x{00EA}",
		'e~' => "\x{1EBD}",
		'i`' => "\x{00EC}",
		'm~' => "m\x{0303}",
		'e&' => "\x{0117}",
		'a~' => "\x{00E3}",
		'a`' => "\x{00E0}",
		'u&' => "u\x{0307}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'u~' => "\x{0169}",
		'u`' => "\x{00F9}",
	},
	'Ebr'        => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'a~' => "\x{00E3}",
		'a&' => "\x{0227}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'n~' => "\x{00F1}",
		'a`' => "\x{00E0}",
		'o^' => "\x{00F4}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'm~' => "m\x{0303}",
		'Ś'  => "\x{A7A8}",
		'u&' => "u\x{0307}",
		'u~' => "\x{0169}",
	},
	'Ef'         => {
		'§'  => "\x{017F}",
		'i`' => "\x{00EC}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'e~' => "\x{1EBD}",
		'i^' => "\x{00EE}",
		'e#' => "\x{00EB}",
		'a`' => "\x{00E0}",
		'o^' => "\x{00F4}",
		'Ś'  => "\x{A7A8}",
		'n~' => "\x{00F1}",
	},
	'Fil'        => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'i^' => "\x{00EE}",
		'e&' => "\x{0117}",
		'a&' => "\x{0227}",
		'a`' => "\x{00E0}",
		'i`' => "\x{00EC}",
		'Ś'  => "\x{A7A8}",
		'o^' => "\x{00F4}",
	},
	'Flm'        => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'Ś'  => "\x{A7A8}",
	},
	'Gal'        => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a&' => "\x{0227}",
		'a^' => "\x{00E2}",
		'a~' => "\x{00E3}",
		'i`' => "\x{00EC}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'a`' => "\x{00E0}",
		'm~' => "m\x{0303}",
		'Ś'  => "\x{A7A8}",
		'i^' => "\x{00EE}",
	},
	'Jk'         => {
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a~' => "\x{00E3}",
		'a&' => "\x{0227}",
		'a^' => "\x{00E2}",
		'e~' => "\x{1EBD}",
		'u~' => "\x{0169}",
		'a`' => "\x{00E0}",
		'u&' => "u\x{0307}",
		'o^' => "\x{00F4}",
		'n~' => "\x{00F1}",
		'e`' => "\x{00E8}",
		'm~' => "m\x{0303}",
		'Ś'  => "\x{A7A8}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
	},
	'Jn'         => {
		'a^' => "\x{00E2}",
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a~' => "\x{00E3}",
		'a&' => "\x{0227}",
		'o&' => "\x{022F}",
		'a`' => "\x{00E0}",
		'i^' => "\x{00EE}",
		'e#' => "\x{00EB}",
		'i#' => "\x{00EF}",
		'e&' => "\x{0117}",
		'o`' => "\x{00F2}",
		'o^' => "\x{00F4}",
		'e^' => "\x{00EA}",
		'a#' => "\x{00E4}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
		'n~' => "\x{00F1}",
		'm~' => "m\x{0303}",
		'Ś'  => "\x{A7A8}",
		'o#' => "\x{00F6}",
		'e~' => "\x{1EBD}",
	},
	'Jud'        => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a&' => "\x{0227}",
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'a~' => "\x{00E3}",
		'e~' => "\x{1EBD}",
		'u~' => "\x{0169}",
	},
	'Kol'        => {
		'a^' => "\x{00E2}",
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'i^' => "\x{00EE}",
		'a`' => "\x{00E0}",
		'i`' => "\x{00EC}",
		'o^' => "\x{00F4}",
		'a&' => "\x{0227}",
		'o&' => "\x{022F}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
	},
	'Lk'         => {
		'a^' => "\x{00E2}",
		'a`' => "\x{00E0}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a&' => "\x{0227}",
		'i^' => "\x{00EE}",
		'o^' => "\x{00F4}",
		'o`' => "\x{00F2}",
		'e&' => "\x{0117}",
		'e^' => "\x{00EA}",
		'a#' => "\x{00E4}",
		'o&' => "\x{022F}",
		'i`' => "\x{00EC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'u#' => "\x{00FC}",
		'n~' => "\x{00F1}",
		'e~' => "\x{1EBD}",
		'u~' => "\x{0169}",
		'Ś'  => "\x{A7A8}",
		'a~' => "\x{00E3}",
		'u^' => "\x{00FB}",
	},
	'Mk'         => {
		'i`' => "\x{00EC}",
		'a^' => "\x{00E2}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'o^' => "\x{00F4}",
		'i^' => "\x{00EE}",
		'e&' => "\x{0117}",
		'o&' => "\x{022F}",
		'a&' => "\x{0227}",
		'a`' => "\x{00E0}",
		'e#' => "\x{00EB}",
		'e`' => "\x{00E8}",
		'e^' => "\x{00EA}",
		'o`' => "\x{00F2}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
		'a~' => "\x{00E3}",
		'u^' => "\x{00FB}",
		'Ś'  => "\x{A7A8}",
		'u~' => "\x{0169}",
	},
	'Mt'         => {
		'a^' => "\x{00E2}",
		'ś'  => "\x{1E9C}",
		'§'  => "\x{017F}",
		'a~' => "\x{00E3}",
		'o^' => "\x{00F4}",
		'o`' => "\x{00F2}",
		'i^' => "\x{00EE}",
		'e^' => "\x{00EA}",
		'a#' => "\x{00E4}",
		'i`' => "\x{00EC}",
		'e`' => "\x{00E8}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'a`' => "\x{00E0}",
		'a&' => "\x{0227}",
		'e~' => "\x{1EBD}",
		'm~' => "m\x{0303}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
		'a´' => "\x{00E1}",
		'n~' => "\x{00F1}",
		'Ś'  => "\x{A7A8}",
		'u~' => "\x{0169}",
		'u^' => "\x{00FB}",
	},
	'Prolog_Iev' => {
		'a`'  => "\x{00E0}",
		'a´'  => "\x{00E1}",
		'a^'  => "\x{00E2}",
		'i^' => "\x{00EE}",
		'm~' => "m\x{0303}",
		'o^'  => "\x{00F4}",
		'§'   => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u&'  => "u\x{0307}",
	},
	'Prolog_Sat' => {
		'§'   => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'Prolog_Tit' => {
		'§'   => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'Rm'   	     => {
		'a^' => "\x{00E2}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a&' => "\x{0227}",
		'i^' => "\x{00EE}",
		'a~' => "\x{00E3}",
		'o^' => "\x{00F4}",
		'n~' => "\x{00F1}",
		'o`' => "\x{00F2}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'a`' => "\x{00E0}",
		'u&' => "u\x{0307}",
		'e~' => "\x{1EBD}",
		'i`' => "\x{00EC}",
		'e`' => "\x{00E8}",
		'Ś'  => "\x{A7A8}",
	},
	'Tit'        => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'i`' => "\x{00EC}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
	},
};

our $ST_VD1689_94 = {
	'1Ken'   => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'o`' => "\x{00F4}",
		'u~' => "\x{00FC}",
	},
	'1L'     => {
		'a`' => "\x{00E0}",
		'a´' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'a#' => "\x{00E4}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'm~' => "m\x{0303}",
		'o`' => "\x{00F2}",
		'o^' => "\x{00F4}",
		'o&' => "\x{022F}",
		'o#' => "\x{00F6}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
	},
	'1Moz'   => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'o#' => "\x{00F6}",
		'o&' => "\x{022F}",
		'o`' => "\x{00F2}",
		'i#' => "\x{00EF}",
		'e`' => "\x{00E8}",
		'i`' => "\x{00EC}",
		'a~' => "\x{00E3}",
		'u^' => "\x{00FB}",
	},
	'1Sam'   => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'e~'  => "\x{1EBD}",
		'u^' => "\x{00FB}",
		'i#' => "\x{00EF}",
		'a´' => "\x{00E1}",
	},
	'2Ken'   => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'o#' => "\x{00F6}",
		'n`' => "\x{01F9}",
	},
	'2L'     => {
		'a`' => "\x{00E0}",
		'a^' => "\x{00E2}",
		'a&' => "\x{0227}",
		'a#' => "\x{00E4}",
		'e~' => "\x{1EBD}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'o^' => "\x{00F4}",
		'o#' => "\x{00F6}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś' => "\x{A7A8}",
		'S`' => "S",		#Datu kļūdas labojums
		'u^' => "\x{00FB}",
		'u&' => "u\x{0307}",
		'u#' => "\x{00FC}",
	},
	'2Moz'   => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'o#' => "\x{00F6}",
		'o&' => "\x{022F}",
		'i`' => "\x{00EC}",
		'a~' => "\x{00E3}",
		'e~' => "\x{1EBD}",
		'u^' => "\x{00FB}",
		'e´' => "\x{00E9}",
	},
	'2Sam'   => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'e~' => "\x{1EBD}",
		'u^' => "\x{00FB}",
		'o#' => "\x{00F6}",
		'o&' => "\x{022F}",
	},
	'3Moz'   => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'e#' => "\x{00EB}",
		'u^' => "\x{00FB}",
		'i#' => "\x{00EF}",
		'i`' => "\x{00EC}",
	},
	'4Moz'   => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'e~' => "\x{1EBD}",
		'u^' => "\x{00FB}",
		'i#' => "\x{00EF}",
		'A`' => "\x{00C0}",
	},
	'5Moz'   => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'o`' => "\x{00F2}",
		'o&' => "\x{022F}",
		'i#' => 'i',
		'u&' => "u\x{0307}",
	},
	'Am'     => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'm~' => "m\x{0303}",
		'u´' => "\x{00FA}",
		'o`' => "\x{00F2}",
		'a´' => "\x{00E1}",
	},
	'Cef'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'Ś' => "\x{A7A8}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'u^' => "\x{00FB}",
		'u´' => "\x{00FA}",
		'n~' => "\x{00F1}",
		'i^' => "\x{00EE}",
	},
	'Dan'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'e~' => "\x{1EBD}",
		'o`' => "\x{00F2}",
		'o#' => "\x{00F6}",
		'n~' => "\x{00F1}",
		'i#' => "\x{00EF}",
		'ģ~' => "Ģ\x{0303}",
		'Ģ~' => "Ģ\x{0303}",
	},
	'Dz'     => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'u#' => "\x{00FC}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
	},
	'Ech'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'u#' => "\x{00FC}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'a´' => "\x{00E1}",
		'u&' => "u\x{0307}",
		'e~' => "\x{1EBD}",
		'o`' => "\x{00F2}",
		'o#' => "\x{00F6}",
		'u´' => "\x{00FA}",
	},
	'Est'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'm~' => "m\x{0303}",
		'u#' => "\x{00FC}",
		'i`' => "\x{00EC}",
		'o`' => "\x{00F2}",
	},
	'Ezr'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'o`' => "\x{00F2}",
		'e~' => "\x{1EBD}",
		'a#' => "a\x{00EB}",
	},
	'Hab'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'Ś' => "\x{A7A8}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'u^' => "\x{00FB}",
		'n~' => "\x{00F1}",
	},
	'Hag'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'Ś' => "\x{A7A8}",
		'e#' => "\x{00EB}",
		'e&' => "\x{0117}",
		'u^' => "\x{00FB}",
		'i^' => "\x{00EE}",
	},
	'Hoz'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'e~' => "\x{1EBD}",
		'n~' => "\x{00F1}",
		'a~' => "\x{00E3}",
		'u&' => "u\x{0307}",
		'u´' => "\x{00FA}",
		'u`' => "\x{00F9}",
	},
	'Ij'     => {
		'a`' => "\x{00E0}",
		'a~' => "\x{00E3}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
	},
	'Jer'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'a´' => "\x{00E1}",
		'u&' => "u\x{0307}",
		'e~' => "\x{1EBD}",
	},
	'Jes'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'a´' => "\x{00E1}",
		'u&' => "u\x{0307}",
		'A&' => "\x{0226}",
	},
	'Jl'     => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
	},
	'Jon'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'm~' => "m\x{0303}",
		'e^' => "\x{00EA}",
	},
	'Joz'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'i#' => "\x{00EF}",
		'o#' => "\x{00F6}",
		'u~' => "\x{0169}",
		'e`' => "\x{00E8}",
		'e~' => "\x{0117}",
		'o`' => "\x{00F4}",
	},
	'Mac'    => {
		'a`' => "\x{00E0}",
		'a\'' => "\x{00E1}",
		'a^' => "\x{00E2}",
		'e^' => "\x{00EA}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'i^' => "\x{00EE}",
		'm~' => "m\x{0303}",
		'o^' => "\x{00F4}",
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
		'u^' => "\x{00FB}",
	},
	'Mal'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
	},
	'Mih'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'm~' => "m\x{0303}",
		'e^' => "\x{00EA}",
		'a#' => "\x{00E4}",
		'e&' => "\x{0117}",
		'u^' => "\x{00FB}",
		'u´' => "\x{00FA}",
	},
	'Nah'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'e#' => "\x{00EB}",
		'm~' => "m\x{0303}",
		'e^' => "\x{00EA}",
		'u^' => "\x{00FB}",
		'a&' => "\x{0227}",
	},
	'Neh'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'a´' => "\x{00E1}",
		'o#' => "\x{00F6}",
		'o&' => "\x{022F}",
		'u^' => "\x{00FB}",
	},
	'Ob'     => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
	},
	'Prolog' => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'Ś'  => "\x{A7A8}",
	},
	'Ps'     => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
		'e\'' => "e\x{1E9C}",
		'i´' => "\x{00ED}",
		'A&' => "\x{0226}",
	},
	'Rdz'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
	},
	'Rut'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
	},
	'Sak'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'e&' => "\x{0117}",
		'u#' => "\x{00FC}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
	},
	'Sog'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'm~' => "m\x{0303}",
		'n~' => "\x{00F1}",
		'u#' => "\x{00FC}",
		'u&' => "u\x{0307}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'e~'  => "\x{1EBD}",
		'u^' => "\x{00FB}",
		'i#' => "\x{00EF}",
		'u~' => "\x{0169}",
		'e`' => "\x{00E8}",
		'u`' => "\x{00F9}",
	},
	'Zah'    => {
		'§'  => "\x{017F}",
		'ś'  => "\x{1E9C}",
		'a^' => "\x{00E2}",
		'o^' => "\x{00F4}",
		'a`' => "\x{00E0}",
		'e^' => "\x{00EA}",
		'i^' => "\x{00EE}",
		'Ś' => "\x{A7A8}",
		'a#' => "\x{00E4}",
		'a&' => "\x{0227}",
		'e&' => "\x{0117}",
		'e#' => "\x{00EB}",
		'i`' => "\x{00EC}",
		'u^' => "\x{00FB}",
	},
	#### Beidzas VD1689_94
};

sub substTable
{
	my $tableName = shift @_;
	my $collection = shift @_;
	return $ST_SINGLES->{$tableName} unless ($collection);
	return $ST_Apokr1689->{$tableName} if ($collection =~ /\s*Apokr1689\s*/);
	return $ST_JT1685->{$tableName} if ($collection =~ /\s*JT1685\s*/);
	return $ST_VD1689_94->{$tableName} if ($collection =~ /\s*VD1689_94\s*/);
	return $ST_SINGLES->{$tableName};
}

sub hasTable
{
	my $tableName = shift @_;
	my $collection = shift @_;
	return exists $ST_SINGLES->{$tableName} unless ($collection);
	return exists $ST_Apokr1689->{$tableName} if ($collection =~ /\s*Apokr1689\s*/);
	return exists $ST_JT1685->{$tableName} if ($collection =~ /\s*JT1685\s*/);
	return exists $ST_VD1689_94->{$tableName} if ($collection =~ /\s*VD1689_94\s*/);
	return exists $ST_SINGLES->{$tableName};
}

1;